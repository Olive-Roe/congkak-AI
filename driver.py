...
def playGame(replayFile="", player1="simple", player2="simple", replayName=""):
    moveList = []
    turn = 1
    board = setupBoard()
    p1pit, p2pit = 0, 0
    ...
    if replayFile != "":
        with open(f"replays/{replayFile}.txt", "r") as f:
    ...
    win = "No one" if p1pit == p2pit else "Player 1" if p1pit > p2pit else "Player 2"
    print(f"{win} wins.")
    if replayFile == "":
        if replayName == "":
            print(moveList)
            while True:
                ...
                if not(exists(f"replays/{replayName}.txt")):
                    break
                else:
                    print("This file name is already taken.")
        with open(f"replays/{replayName}.txt", "w") as f:
